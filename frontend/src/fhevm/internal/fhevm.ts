...
async function getChainId(
  ...
  if (!providerOrUrl || typeof providerOrUrl.request !== "function") {
    throw new Error(
      "Invalid EIP-1193 provider: provider.request is not a function. " +
      "Please ensure you are passing a valid EIP-1193 provider (e.g., window.ethereum)."
    );
  }
...
async function resolve(
  ...
  // Resolve chainId
  const chainId = chainIdOverride ?? await getChainId(providerOrUrl);
...
export const createFhevmInstance = async (parameters: {
  ...
  const { signal, onStatusChange, provider: providerOrUrl, mockChains, chainId: chainIdOverride } = parameters;
  ...
  // Resolve chainId
  const { isMock, rpcUrl, chainId } = await resolve(providerOrUrl, mockChains, chainIdOverride);
  ...
  const relayerSDK = (window as unknown as FhevmWindowType).relayerSDK as any;
